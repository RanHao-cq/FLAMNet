...
from flamnet.utils.lane import Lane
from flamnet.models.losses.focal_loss import FocalLoss
from flamnet.models.losses.accuracy import accuracy
from flamnet.ops import nms

from flamnet.models.utils.roi_gather import anchor_pre, LinearModule,HIAM
from flamnet.models.utils.seg_decoder import SegDecoder
from flamnet.models.utils.dynamic_assign import assign
from flamnet.models.losses.lineiou_loss import liou_loss
from ..registry import HEADS


@HEADS.register_module
class FLAMNetHead(nn.Module):
    def __init__(self,
                 num_points=72,
                 ...
                 sample_points=36,
                 cfg=None):
        super(FLAMNetHead, self).__init__()
        self.cfg = cfg
        ...
        self.register_buffer(name='fea_x_indexs', tensor=torch.linspace(
            0, 1, steps=self.num_priors, dtype=torch.float32))

        self.register_buffer(name='fea_y_indexs', tensor=torch.linspace(
            0, 1, steps=self.num_priors, dtype=torch.float32))

        self.register_buffer(name='prior_feat_ys', tensor=torch.flip(
            (1 - self.sample_x_indexs.float() / self.n_strips), dims=[-1]))

        self.h_prior_feat_ys = self.prior_feat_ys[18:]
        self.h_prior_feat_ys = self.h_prior_feat_ys.cuda()
        ...
        self.roi_gather = HIAM(self.prior_feat_channels, self.num_priors,
                                    self.sample_points, self.fc_hidden_dim,
                                    self.refine_layers)
        self.anchor_pre = anchor_pre(self.prior_feat_channels, self.num_priors,
                                    self.sample_points, self.fc_hidden_dim,
                                    self.refine_layers)

        self.refine_reg_layers = nn.Linear(
            self.fc_hidden_dim, self.n_offsets//2)
    ...
    def pool_prior_features(self, batch_features, num_priors, prior_xs):
        ...
        fea_xs = self.fea_x_indexs.repeat(batch_size, self.sample_points // 2, 1).view(batch_size,
                                                                                       self.sample_points // 2, -1,
                                                                                       1)  # *********
        fea_ys = self.h_prior_feat_ys.repeat(batch_size, self.num_priors, 1).view(batch_size, -1,
                                                                                  self.sample_points // 2,
                                                                                  1).permute(0, 2, 1, 3)  # ********
        fea_grid = torch.cat((fea_xs, fea_ys), dim=-1)
    ...
    def forward(self, x, **kwargs):
        ...
        for stage in range(self.refine_layers):
            ...
            batch_prior_features, H_fea = self.pool_prior_features(
                batch_features[stage], num_priors, prior_xs)
            prior_features_stages.append(batch_prior_features)

            fc_features = self.anchor_pre(prior_features_stages, batch_features[stage], stage)

            fc_features = fc_features.view(num_priors, batch_size,
                                           -1).reshape(batch_size * num_priors,
                                                       self.fc_hidden_dim)
            refine_fc_features = self.roi_gather(prior_features_stages, H_fea, stage)
            ...
            refine = self.refine_reg_layers(refine_fc_features)
