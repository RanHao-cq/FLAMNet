...
import flamnet.utils.culane_metric as culane_metric
import cv2
...
class CULane(BaseDataset):
    ...
    def evaluate(self, predictions, output_basedir):
        ...
        for cate, cate_file in CATEGORYS.items():
            ...
            TOtal_TP += result[0.5]['TP']
            TOtal_FP += result[0.5]['FP']
            TOtal_FN += result[0.5]['FN']

        TOtal_P = TOtal_TP * 1.0 / (TOtal_TP + TOtal_FP + 1e-9)
        TOtal_R = TOtal_TP * 1.0 / (TOtal_TP + TOtal_FN + 1e-9)
        TOtal_F = 2 * TOtal_P * TOtal_R / (TOtal_P + TOtal_R + 1e-9)

        logger.info('precision: {}, recall: {}, f1: {}'.format(TOtal_P, TOtal_R, TOtal_F))

        # result = culane_metric.eval_predictions(output_basedir,
        #                                         self.data_root,
        #                                         self.list_path,
        #                                         iou_thresholds=np.linspace[0.5],
        #                                         official=True)
